...
export const { data, execute, meta } = new CommandBuilder()
	.setName("welcome")
	.setDescription("Configure the welcome message system for this server.")
	.setCategory("automod")
	.setUsage("/welcome <enable|disable|setchannel|setmessage|status>")
	.setGuard("guild")
	.setOptions((builder) =>
		builder
			.addSubcommand((sub) =>
		...
		switch (sub) {
			...
			case "setchannel": {
				...
				return interaction.reply({
					embeds: [
						successEmbed(
							`Welcome channel set to ${channel}.\nUse \`/welcome enable\` to activate it.`
						),
					],
				});
			}
			...
			case "status":
			default: {
				const status = server?.welcome?.enabled
					? "🟢 Enabled"
					: "🔴 Disabled";
