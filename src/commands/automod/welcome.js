import { loadGuild } from "#database/schema";
import {
	setWelcomeChannel,
	setWelcomeEnabled,
	setWelcomeMessage,
...
class WelcomeCommand extends Command {
	constructor() {
		super(
			"welcome",
			"Configure the welcome message system for this server.",
			{
				category: "automod",
		...
		switch (sub) {
			case "enable": {
				...
				return interaction.reply({
					embeds: [
						successEmbed("Welcome system has been **enabled**."),
					],
				});
			}
			...
			case "disable": {
				...
				return interaction.reply({
					embeds: [
						successEmbed("Welcome system has been **disabled**."),
					],
				});
			}
			...
			case "setchannel": {
				...
				return interaction.reply({
					embeds: [
						successEmbed(`Welcome channel set to ${channel}.`),
					],
				});
			}
			...
			default: {
				const db = loadGuild(guildId);
				const status = db.welcome.enabled
					? "🟢 Enabled"
					: "🔴 Disabled";
				const ch = db.welcome.channelId
					? `<#${db.welcome.channelId}>`
					: "*not set*";

				return interaction.reply({
					embeds: [
						infoEmbed(
							`**Status:** ${status}\n**Channel:** ${ch}\n**Message:** ${db.welcome.message}`,
